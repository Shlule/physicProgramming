...
bool Intersection::Intersect(Body& a, Body& b,const float dt, Contact& contact) {
	...
	if (a.shape->GetType() == Shape::ShapeType::SHAPE_SPHERE &&
		...
		Vec3 velA = a.linearVelocity;
		Vec3 velB = b.linearVelocity;

		if (Intersection::SphereSphereDynamic(*sphereA, *sphereB, posA, posB, velA, velB, dt, contact.ptOnAWorldSpace, contact.ptOnBWorldSpace, contact.timeOfImpact)) {
...
bool Intersection::RaySphere(const Vec3& rayStart, const Vec3& rayDir, const Vec3& sphereCenter, const float spherRadius, float& t0, float& t1) {
	const Vec3& s = sphereCenter - rayStart;
	const float a = rayDir.Dot(rayDir);
	const float b = s.Dot(rayDir);
	const float c = s.Dot(s) - spherRadius * spherRadius;

	const float delta = b * b - a * c;
	const float inverseA = 1.0f / a;

	if (delta < 0) {
		// No solution , no colliding
		return false;
	}
